...
void main()
{
	...
	for (int i = 0; i<inputimage->height; i++)
	{
		for (int j = 0; j<inputimage->width; j++)
		{
			...
			{
				outputimage->imageData[i*outputimage->widthStep+j*outputimage->nChannels+h] = 
					inputimage->imageData[i*inputimage->widthStep+(inputimage->width-1-j)*inputimage->nChannels+h];
			}
		}
	}
