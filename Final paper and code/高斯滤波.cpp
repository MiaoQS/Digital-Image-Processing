...
void main()
{
	...
	for (int d = 1; d< 10; d++)
	{
		...
		for (int i = 0; i < kuozhan->height; i++)
		{
			for (int k = 0; k < kuozhan->nChannels; k++)
			{
				...
				kuozhan->imageData[(i+1)*kuozhan->widthStep-kuozhan->nChannels+k] = kuozhan->imageData[(i+1)*kuozhan->widthStep-2 * kuozhan->nChannels+k];
			}
		}
	...
	cvSaveImage("gaosigshmbb.jpg", gaosismooth);
